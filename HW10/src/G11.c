 ...
 * G11: В файле input.txt дано предложение. Необходимо определить,
...
#define CAT_PATH CATALOG_FILES_DEF_HW10 "G11/"
#else
#define CAT_PATH ""
#endif
...
char* read_file_g11 (const char *file_name, char buffer[], const int size)
{
    if (buffer == NULL)
    {
...
int write_to_file_g11 (const char *file_name, int num)
{
    FILE *file = NULL;
...
/**
 * @brief Разбить строку на части по разделителю и вернуть кол-во слов
 *        которые заканчиваются на 'а'.
 ...
 * @return Кол-во слов которые заканчиваются на 'а'.
 * 
 */
int ends_with_a (const char *string, const char *delimeter)
{
...
int main (void)
{
    ...
    if (read_file_g11(FILE_INPUT, buffer, MAX_LEN) == NULL)
    {
        return EXIT_FAILURE;
    }
    int end_a = ends_with_a(buffer, " ");
    write_to_file_g11(FILE_OUTPUT, end_a);
