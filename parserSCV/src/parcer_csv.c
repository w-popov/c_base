...
struct ContextParser *parse_csv (struct ContextParser *context, FILE *file)
{
    if (!context || !file)
    {
        return NULL;
    ...
    while ((character = fgetc(file)) != EOF)
    {
        ...
        if (character == *context->delimiter)
        {
            ...
            {
                context->clb_write_to_arr(NULL, context);
            }
        ...
        else if (character == '\n')
        {
            ...
            {
                context->clb_progress(NULL, ftell(file), context->file_size);
            }
        }
        else
        {
